 ...
 *
 * [ Linux ]
 * compile cmd: gcc -Wl,-rpath,/usr/lib -Wall -fPIC -O2 -o rastertostar rastertostar.c -lcupsimage -lcups -DLINUXNEW
 ...
 *
 * [ Mac OS X ]
 * compile cmd: gcc -Wall -fPIC -o rastertostar rastertostar.c -lcupsimage -lcups -arch ppc -arch i386 -DMACOSX
...
#if defined(MACOSX) || defined(LINUXNEW)
...
void jobSetup(struct settings_ settings, char *argv[])
{
    ...
    if (settings.pageType == PAGETYPE_RECEIPT)
    {
...
void pageSetup(struct settings_ settings, cups_page_header_t header)
{
    ...
    {
        outputCommand(pageTypeCommand[PAGETYPE_TICKET]);
        outputAsciiEncodedLength(header.cupsHeight + 1); // + 1 accomodates trailing a dot Y cmd
        outputNullTerminator();
    }
    ...
    while (CUPSRASTERREADHEADER(ras, &header))
    {
        ...
        for (y = 0; y < header.cupsHeight; y ++)
        {
            ...
            if (scanLineBlank == FALSE)
            {
                ...
                {
                    printf("\x01b*rY%d", numBlankScanLines); putchar(0x00);
                    numBlankScanLines = 0;
                }
